...
class CalduladoraFracciones{
    public fun mostrarMenu() {
    ...
    public fun leerFraccion(scanner: Scanner, mensaje: String): Fraccion {
        println(mensaje)
        print("Numerador: ")
    ...
    public fun realizarSuma (scanner:Scanner) {
        val f1 = leerFraccion(scanner, "Primera fracción")
        val f2 = leerFraccion(scanner, "segunda fracción")
        println("Suma: ${f1+f2}")
    }
    public fun realizarResta (scanner:Scanner) {
        val f1 = leerFraccion(scanner, "Primera fracción")
        val f2 = leerFraccion(scanner, "segunda fracción")
        println("resta: ${f1-f2}")
    }
    public fun realizarDivision (scanner:Scanner) {
        val f1 = leerFraccion(scanner, "Primera fracción")
        val f2 = leerFraccion(scanner, "segunda fracción")
        println("División: ${f1/f2}")
    }
    public fun realizarMultiplicacion (scanner:Scanner) {
        val f1 = leerFraccion(scanner, "Primera fracción")
        val f2 = leerFraccion(scanner, "segunda fracción")
        println("Multiplicación: ${f1*f2}")
    }
    public fun realizarComparacion(scanner:Scanner){
        val f1 = leerFraccion(scanner, "Primera fracción")
        val f2 = leerFraccion(scanner, "segunda fracción")
        when {
    ...
    public fun convertirADecimal(scanner:Scanner) {
        val f1 = leerFraccion(scanner, "Fracción a convertir")
        println("$f1 = ${f1.aDecimal()}")
    }
    public fun crearDesdeDecimal(scanner:Scanner){
